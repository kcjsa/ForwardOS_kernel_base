...
unsafe fn write_sector(abar: u64, port: usize, ct_base: u64, data_ptr: u64, cl_base: u64, lba: u64) {
    if crate::AHCI_PORT == -1 { return; }

    let port_ptr = (abar + 0x100 + (port as u64 * 0x80)) as *mut u32;
    let ct = ct_base as *mut crate::ahci::CommandTable;

    for i in 0..64 { core::ptr::write_volatile(&mut (*ct).cfis[i], 0); }
    core::ptr::write_volatile(&mut (*ct).cfis[0], 0x27);
    core::ptr::write_volatile(&mut (*ct).cfis[1], 0x80);
    core::ptr::write_volatile(&mut (*ct).cfis[2], 0x35);
    ...
    core::ptr::write_volatile(&mut (*ct).cfis[7], 0x40);
    ...
    core::ptr::write_volatile(&mut (*ct).cfis[12], 1);
...
fn alloc_cluster(fs: &SimpleFileSystem) -> Option<u32> {
    ...
    for sector_off in 0..fat_sectors {
        unsafe {
            ahci::read_sector(abar, active_port as usize, fat_lba + sector_off, ct_base, data_ptr, fs.cl_base);
        }
...
unsafe fn set_fat_entry(fs: &SimpleFileSystem, cluster: u32, value: u32) {
    ...
    write_sector(abar, active_port as usize, ct_base, data_ptr, fs.cl_base, fat_sector);
}
...
unsafe fn write_cluster(fs: &SimpleFileSystem, cluster: u32, data: &[u8], offset: u64) -> usize {
    ...
    for i in 0..fs.sectors_per_clus as u64 {
        ...
        ahci::read_sector(abar, active_port as usize, sector_lba, ct_base, data_ptr, fs.cl_base);
        ...
        write_sector(abar, active_port as usize, ct_base, data_ptr, fs.cl_base, sector_lba);
    }
...
pub fn sys_write_file(fd: u32, buf_ptr: u64, buf_len: u64) -> isize {
    ...
    unsafe {
        ...
        if port != -1isize as usize {
            crate::ahci::read_sector(abar, port, 2048, crate::AHCI_CT_BASE, crate::AHCI_DATA_PTR, crate::AHCI_CT_BASE);
        }
    }
        ...
        for i in 0..fs.sectors_per_clus as u64 {
            ...
            for j in 0..16 {
                ...
                if entry_cluster == target_cluster {
                    ...
                    write_sector(abar, active_port as usize, ct_base, data_ptr, fs.cl_base, lba + i);
                    return;
                }
            }
        }
