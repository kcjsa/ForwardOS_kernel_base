...
fn next_cluster(fs: &crate::file::SimpleFileSystem, cluster: u32) -> Option<u32> {
    ...
    unsafe {
        crate::ahci::read_sector(abar, active_port as usize, fat_sector, ct_base, data_ptr);
...
fn read_cluster(fs: &crate::file::SimpleFileSystem, cluster: u32, buf: &mut [u8], offset: u64) -> usize {
    ...
    for i in 0..fs.sectors_per_clus as u64 {
        unsafe {
            crate::ahci::read_sector(abar as u64, active_port as usize, lba + i, ct_base, data_ptr);
        }
...
pub fn sys_read_file(fd: u32, buf_ptr: u64, buf_len: u64) -> isize {
    ...
    let mut buf_pos = 0;
    let mut cur_cluster = current_cluster;  // read_cluster → cur_cluster
    ...
    while remaining > 0 {
        ...
        let n = read_cluster(&fs, cur_cluster, chunk_buf, read_offset);  // ここも
        if n == 0 { break; }
        ...
        if remaining > 0 {
            match next_cluster(&fs, cur_cluster) {  // ここも
...
pub fn syscall_open(path: &str) -> i32 {
    ...
    let fs = unsafe {
        match crate::file::SimpleFileSystem::init(abar, active_port as usize, ct_base, data_ptr) {
            Some(fs) => fs,
            None => return -1,
        }
    };
